...
mulai = time.time()
dangerMessage = ["เอ็มเสียใจด้วยครับ", "โชคดีครับ", "ชีวิตยังมีงวดหน้าเสมอ", ".winebot", ".kickall", "mayhem",
                 "kick on", "makasih :d", "!kickall", "nuke", "บิน", ".???", "Kickall", "บินไปดิ", "Kickword",
                 "kickword", "!kickword", "/kickall"]
dangerMessage2 = ["ว่างัยจ๊ะทิรักของเอ็ม 5555", "คร้าบบ", "ว่างัยน่ะ!!.. รักเอ็มหรออ อิอิ", "ว่างัยเอ๋ยย เอ็มรอฟังอยู่",
                  "เอ็มมาแล้วคร้าบบ", "แอบรักเอ็มอยู่หราาา", "แอบชอบเอ็มหราา", "รักเอ็มหรอ", "คิดถึงเอ็มมากเลยหรอ"]
    ...
    try:
            ...
            if msg.contentType == 0:
                ...
                # ===============หาชื่อล่อนหน===============================================================#
                    ...
                    line.sendMessage(msg.to, (
                        str(datetime.datetime.now() - start_runtime)[:-7].split(" days, ")[0] + " วัน " +
                        str(datetime.datetime.now() - start_runtime)[:-7].split(" days, ")[1].split(":")[
                            0] + " ชั่วโมง " if "days" in str(datetime.datetime.now() - start_runtime) else
                        str(datetime.datetime.now() - start_runtime)[:-7].split(" day, ")[0] + " วัน " +
                        str(datetime.datetime.now() - start_runtime)[:-7].split(" day, ")[1].split(":")[
                            0] + " ชั่วโมง " if "day" in str(datetime.datetime.now() - start_runtime) else
                        str(datetime.datetime.now() - start_runtime)[:-7].split(":")[0] + " ชั่วโมง ") +
                                     str(datetime.datetime.now() - start_runtime)[:-7].split(":")[1] + " นาที " +
                                     str(datetime.datetime.now() - start_runtime)[:-7].split(":")[2] + " วินาที")
                elif "คท " in msg.text:
                ...
                # ==============================================================================#
                    ...
                    try:
                        ...
                        if settings["บอททำงานปกติครับ"] == True:
                            ret_ += "\n╠❂☞ รับบล็อคออโต้ ✔"
                        else:
                            ret_ += "\n╠🔀 รับบล็อคออโต้    ✘ "
                        if settings["autoJoin"] == True:
                        ...
                        if settings["contact"] == True:
                            ...
                            ret_ += "\n╠🔀 เช็คMid ✘"
                        if settings["autoCancel"]["on"] == True:
                            ret_ += "\n╠❂☞ ยกเลิกเชิญกลุ่มเมื่อมีสมาชิกต่ำกว่า: " + str(
                                settings["autoCancel"]["members"]) + " → ✔"
                        else:
                            ret_ += "\n╠🔀 ยกเลิกเชิญกลุ่ม    ✘ "
                        if settings["autoLeave"] == True:
                            ret_ += "\n╠❂☞ ออกแชทรวม ✔"
                ...
                # ==============================================================================#
                    ...
                    line.inviteIntoGroupCall(msg.to, [uid.mid for uid in line.getGroup(msg.to).members if
                                                      uid.mid != line.getProfile().mid])
                    ...
                    if spl[0] == "":
                        ...
                        for i in range(len(prov)):
                            ...
                            userData = line.getContact(uid)
                            try:
                                line.sendImageWithURL(msg.to,
                                                      "http://dl.profile.line.naver.jp/" + userData.pictureStatus)
                    ...
                    if spl[len(spl) - 1] == "":
                        line.sendText(msg.to,
                                      "กดที่นี่เพื่อเขย่าข้อความด้านบน:\nline://nv/chatMsg?chatId=" + msg.to + "&messageId=" + msg.id)
                    ...
                    if spl[0] == "":
                        try:
                            gid = spl[1].split(" ")[0]
                            ticket = spl[1].split(" ")[1].replace("line://ti/g/", "") if "line://ti/g/" in \
                                                                                         spl[1].split(" ")[1] else \
                            spl[1].split(" ")[1].replace("http://line.me/R/ti/g/", "") if "http://line.me/R/ti/g/" in \
                                                                                          spl[1].split(" ")[1] else \
                            spl[1].split(" ")[1]
                    ...
                    try:
                        ...
                        conv = BeautifulSoup(
                            requests.post("http://lullar-de-2.appspot.com/", headers=headers, data=data).content,
                            "html.parser").find("span", attrs={"style": "font-size:40px"}).text
                        if msg.toType != 0:
                    ...
                    if GACReq.responseNum == 0:
                        ...
                        if msg.toType != 0:
                            kk.sendText(msg.to,
                                        "ไม่สามารถส่ง SMS ได้ เนื่องจากมีการส่งข้อความไปยังเบอร์เป้าหมายในเวลาที่ใกล้เคียงกันมากเกินไป \nกรุณารออย่างมาก 30 วินาทีแล้วลองอีกครั้ง")
                        else:
                            kk.sendText(msg.from_,
                                        "ไม่สามารถส่ง SMS ได้ เนื่องจากมีการส่งข้อความไปยังเบอร์เป้าหมายในเวลาที่ใกล้เคียงกันมากเกินไป \nกรุณารออย่างมาก 30 วินาทีแล้วลองอีกครั้ง")
                    else:
                        if msg.toType != 0:
                ...
                # ==============================================================================#
                    ...
                    for a in range(k + 1):
                        ...
                        for i in group.members[a * 100: (a + 1) * 100]:
                            ...
                            s += 7
                            txt += '@Alin \n'
                        line.sendMessage(to, text=txt, contentMetadata={'MENTION': json.dumps({'MENTIONEES': b})},
                                         contentType=0)
                    ...
                    bulan = ["มกราคม", "กุมภาพันธ์", "มีนาคม", "เมษายน", "พฤษภาคม", "มิถุนายน", "กรกฎาคม", "สิงหาคม",
                    ...
                    if receiver in read['readPoint']:
                        ...
                        text = xpesan + zxc + "\n[ Lurking time ]: \n" + readTime
                        try:
                            line.sendMessage(receiver, text, contentMetadata={
                ...
                # ==============================================================================#
                    ...
                    tahun = getfilm[1]
                    r = requests.get(
                        'http://www.omdbapi.com/?t=' + title + '&y=' + tahun + '&plot=full&apikey=4bdd1d70')
                    start = time.time()
                    data = r.text
                    ...
                    with requests.session() as web:
                        r = web.get("http://rahandiapi.herokuapp.com/sswebAPI?key=betakey&link={}".format(
                    ...
                    r = requests.get(
                        'https://script.google.com/macros/exec?service=AKfycbw7gKzP-WYV2F5mc9RaR7yE3Ve1yN91Tjs91hp_jHSE02dSv9w&nama=ervan&tanggal=' + tanggal)
                    data = r.text
                    data = json.loads(data)
                    ret_ = "╔══[ D A T E ]"
                    ...
                    with requests.session() as web:
                        web.headers["User-Agent"] = random.choice(settings["userAgent"])
                        r = web.get("http://rahandiapi.herokuapp.com/imageapi?key=betakey&q={}".format(
                ...
                # =======================================================================================
                    ...
                    line.sendMessage(msg.to, "Update message auto add :" + settings[
                                ...
                                for target in targets:
                                    if not target in Family:
                                        try:
                                            ...
                                            json.dump(settings["blacklist"], f, sort_keys=True, indent=4,
                                                      ensure_ascii=False)
                                        except:
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if sender in settings["mimic"]["target"] and settings["mimic"]["status"] == True and \
                        settings["mimic"]["target"][sender] == True:
                    text = msg.text
                    if text is not None:
                        ...
                        if settings['detectMention'] == True:
                            ...
                            for mention in mentionees:
                                if mention['M'] in lineMID:
                                    ...
                                    line.sendMessage(msg.to, None,
                                                     contentMetadata={"STKID": "51626512", "STKPKGID": "11538",
                                                                      "STKVER": "1"}, contentType=7)
                                    break
                ...
                if msg.text in ["กินข้าวยัง", "กินข้าวยังทุกคน", "กินข้าวกันยัง", "กินข้าวกันยังทุกคน", "กินข้าวยังค่ะ",
                                "กินข้างยังคับ", "กินข้าวยังครับทุกคน"]:
                ...
                if msg.text in dangerMessage:
                    ...
                    random.choice(Rfu).sendText(msg.to,
                                                "ตรวจพบคำสั่งของบอทลบกลุ่ม  เอ็ม Bot Self จำเป็นต้องนำออกเพื่อความปลอดภัยของสมาชิก")
        if op.type == 17:
            if settings["Wc"] == True:
                ...
                line.sendText(op.param1,
                              "" + line.getContact(op.param2).displayName + " ☞ " + str(ginfo.name) + " ☜""\n\n" + str(
                                  settings["man1"]))
                ...
                if RfuCctv['cyduk'][op.param1] == True:
                    if op.param1 in RfuCctv['point']:
                        ...
                        if Name in RfuCctv['sidermem'][op.param1]:
                            ...
                            pref = ['สวัสดีครับคนอ่าน➡', 'ตร๊ะ!..เอ๋!!..😝➡', 'หวัดดีจร้า💖➡', 'เอ็มเห็นนะ อิอิ..😝➡',
                                    'แอบทำมัย? มาคุยกันหน่อย➡', 'แอบแฟนเล่นหราา💨➡', 'เอ็มรู้นะว่าคัยอ่าน👉➡',
                                    'เล่นซ่อนแอบหรอคร้าบบ👻➡', 'มาคุยกับเค้าหน่อย➡', 'นั่นแน่ แอบหราาา➡']
